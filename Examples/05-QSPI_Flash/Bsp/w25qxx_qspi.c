...
uint16_t w25qxx_GetID(void)
{
	...
	if(w25qxx_Mode == w25qxx_SPIMode)
		QSPI_Send_CMD(&hqspi1,W25X_QUAD_ManufactDeviceID,0x00,QSPI_ADDRESS_24_BITS,6,QSPI_INSTRUCTION_1_LINE,QSPI_ADDRESS_4_LINES, QSPI_DATA_4_LINES, sizeof(ID));
	else
		QSPI_Send_CMD(&hqspi1,W25X_ManufactDeviceID,0x00,QSPI_ADDRESS_24_BITS,0,QSPI_INSTRUCTION_4_LINES,QSPI_ADDRESS_4_LINES, QSPI_DATA_4_LINES, sizeof(ID));
...
uint8_t w25qxx_ReadSR(uint8_t SR)
{
	uint8_t byte=0;
	if(w25qxx_Mode == w25qxx_SPIMode)
		QSPI_Send_CMD(&hqspi1,SR,0x00,QSPI_ADDRESS_8_BITS,0,QSPI_INSTRUCTION_1_LINE,QSPI_ADDRESS_NONE, QSPI_DATA_1_LINE, 1);
	else
		QSPI_Send_CMD(&hqspi1,SR,0x00,QSPI_ADDRESS_8_BITS,0,QSPI_INSTRUCTION_4_LINES,QSPI_ADDRESS_NONE, QSPI_DATA_4_LINES, 1);
	
	if (HAL_QSPI_Receive(&hqspi1,&byte,HAL_QSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
	{
...
uint8_t w25qxx_WriteSR(uint8_t SR,uint8_t data)
{
  ...
  return HAL_QSPI_Transmit(&hqspi1,&data,HAL_QSPI_TIMEOUT_DEFAULT_VALUE);
}
...
uint8_t w25qxx_SetReadParameters(uint8_t DummyClock,uint8_t WrapLenth)
{
	...
	QSPI_Send_CMD(&hqspi1,W25X_SetReadParam,0x00,QSPI_ADDRESS_8_BITS,0,QSPI_INSTRUCTION_4_LINES,QSPI_ADDRESS_NONE, QSPI_DATA_4_LINES, 1);
	
	return HAL_QSPI_Transmit(&hqspi1,&send,HAL_QSPI_TIMEOUT_DEFAULT_VALUE);
}

uint8_t w25qxx_EnterQPI(void)
{
	/* Enter QSPI memory in QSPI mode */
  if(QSPI_EnterQPI(&hqspi1) != w25qxx_OK)
  {
    return w25qxx_ERROR;
  }
...
uint8_t w25qxx_Startup(uint8_t DTRMode)
{
  ...
  if( QSPI_EnableMemoryMappedMode(&hqspi1,DTRMode) != w25qxx_OK )
  {
    return w25qxx_ERROR;
  }
  return w25qxx_OK;
...
{
	return QSPI_WriteEnable(&hqspi1);
}
/**
...
uint8_t W25qxx_EraseSector(uint32_t SectorAddress)
{
	...
	if(w25qxx_Mode == w25qxx_SPIMode)
		result = QSPI_Send_CMD(&hqspi1,W25X_SectorErase,SectorAddress,QSPI_ADDRESS_24_BITS,0,QSPI_INSTRUCTION_1_LINE,QSPI_ADDRESS_1_LINE,QSPI_DATA_NONE,0);
  else
		result = QSPI_Send_CMD(&hqspi1,W25X_SectorErase,SectorAddress,QSPI_ADDRESS_24_BITS,0,QSPI_INSTRUCTION_4_LINES,QSPI_ADDRESS_4_LINES,QSPI_DATA_NONE,0);
...
uint8_t W25qxx_EraseBlock(uint32_t BlockAddress)
{
	...
	if(w25qxx_Mode == w25qxx_SPIMode)
		result = QSPI_Send_CMD(&hqspi1,W25X_BlockErase,BlockAddress,QSPI_ADDRESS_24_BITS,0,QSPI_INSTRUCTION_1_LINE,QSPI_ADDRESS_1_LINE,QSPI_DATA_NONE,0);
  else
		result = QSPI_Send_CMD(&hqspi1,W25X_BlockErase,BlockAddress,QSPI_ADDRESS_24_BITS,0,QSPI_INSTRUCTION_4_LINES,QSPI_ADDRESS_4_LINES,QSPI_DATA_NONE,0);
...
uint8_t W25qxx_EraseChip(void)
{
	...
	if(w25qxx_Mode == w25qxx_SPIMode)
		result = QSPI_Send_CMD(&hqspi1,W25X_ChipErase,0x00,QSPI_ADDRESS_8_BITS,0,QSPI_INSTRUCTION_1_LINE,QSPI_ADDRESS_NONE,QSPI_DATA_NONE,0);
  else
		result = QSPI_Send_CMD(&hqspi1,W25X_ChipErase,0x00,QSPI_ADDRESS_8_BITS,0,QSPI_INSTRUCTION_4_LINES,QSPI_ADDRESS_NONE,QSPI_DATA_NONE,0);
...
uint8_t W25qxx_PageProgram(uint8_t *pData, uint32_t WriteAddr, uint32_t Size)
{
	...
	if(w25qxx_Mode == w25qxx_SPIMode)
		result = QSPI_Send_CMD(&hqspi1,W25X_QUAD_INPUT_PAGE_PROG_CMD,WriteAddr,QSPI_ADDRESS_24_BITS,0,QSPI_INSTRUCTION_1_LINE,QSPI_ADDRESS_1_LINE,QSPI_DATA_4_LINES,Size);
  else
		result = QSPI_Send_CMD(&hqspi1,W25X_PageProgram,WriteAddr,QSPI_ADDRESS_24_BITS,0,QSPI_INSTRUCTION_4_LINES,QSPI_ADDRESS_4_LINES,QSPI_DATA_4_LINES,Size);
	
	if(result == w25qxx_OK)
		result = HAL_QSPI_Transmit(&hqspi1,pData,HAL_QSPI_TIMEOUT_DEFAULT_VALUE);
...
uint8_t W25qxx_Read(uint8_t *pData, uint32_t ReadAddr, uint32_t Size)
{
	...
	result = HAL_QSPI_Command(&hqspi1, &s_command, HAL_QSPI_TIMEOUT_DEFAULT_VALUE);
	
	if(result == w25qxx_OK)
		result = HAL_QSPI_Receive(&hqspi1,pData,HAL_QSPI_TIMEOUT_DEFAULT_VALUE);
                    ...
                    uint32_t instructionMode,uint32_t addressMode, uint32_t dataMode, uint32_t dataSize)
{
    ...
    if(HAL_QSPI_Command(hqspi, &Cmdhandler, HAL_QSPI_TIMEOUT_DEFAULT_VALUE) != HAL_OK)
      return w25qxx_ERROR;
...
static uint32_t QSPI_EnterFourBytesAddress(QSPI_HandleTypeDef *hqspi)
{
  ...
  if (QSPI_AutoPollingMemReady(hqspi, HAL_QSPI_TIMEOUT_DEFAULT_VALUE) != w25qxx_OK)
  {
    return w25qxx_ERROR;
  }
